...
exports.lessons = {
    ...
    updIss: data => {
        return new Promise((resolve, reject) => {
            start(connection => {
                const sql = 'UPDATE `lessons` SET `in_iss` = ? WHERE `id` = ?';
                connection.query(sql, [data.in_iss, data.id],
                    (err, results) => {
                        if (err) reject(err)
                        else resolve(results)
                        end()
    ...
    updContent: data => {
        return new Promise((resolve, reject) => {
            start(connection => {
                const sql = 'UPDATE `lessons` SET `content` = ? WHERE `id` = ?';
                console.log(data)
                connection.query(sql, [data.content, data.id],
                    (err, results) => {
                        if (err) reject(err)
                        else resolve(results)
                        end()
